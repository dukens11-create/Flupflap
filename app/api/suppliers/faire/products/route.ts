...
export async function GET(req: Request) {
  ...
  try {
    const result = await runSupplierProductSync('FAIRE', {
      userId: session.user.id,
      path: new URL(req.url).pathname,
      method: 'GET',
    });
    return supplierSuccessResponse(result, 'FAIRE', 'products');
  } catch (error) {
    ...
    return supplierErrorResponse({
      ...
      message: error instanceof Error ? error.message : 'Failed to sync Faire products.',
    });
  }
}
