...
export async function POST(
  ...
  try {
    ...
    return NextResponse.redirect(new URL('/admin/reports?success=Report+moderated.', req.url));
  } catch (err: any) {
    if (err?.name === 'ZodError') {
      if (req.headers.get('accept')?.includes('application/json')) {
        return NextResponse.json({ error: 'Invalid input — please select an action.' }, { status: 400 });
      }
      const errUrl = new URL('/admin/reports', req.url);
    ...
    if (req.headers.get('accept')?.includes('application/json')) {
      return NextResponse.json({ error: 'Failed to process report. Please try again.' }, { status: 500 });
    }
    const errUrl = new URL('/admin/reports', req.url);
    errUrl.searchParams.set('error', 'Failed to process report. Please try again.');
    return NextResponse.redirect(errUrl, 302);
  }
}
