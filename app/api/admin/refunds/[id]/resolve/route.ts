...
import { AdminRefundActionError, resolveRefundRequest } from '@/lib/admin-refunds';
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const updated = await resolveRefundRequest({
      id,
    ...
    console.error(`Failed to resolve refund ${id}:`, error);
